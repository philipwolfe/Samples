...
void DumpPrivileges(CTokenPrivileges& priv)
{
	...
	for (UINT i=0; i<priv.GetCount(); i++)
	{
		...
		Log(_T("Name: %s"), names[i]);
		Log(_T("Display Name: %s"), dnames[i]);
		Log(_T("Attributes:"));
...
void DumpStatistics(TOKEN_STATISTICS& stats)
{
	...
	Log(_T("TokenType: %s"), GetTokenType(stats.TokenType));
	Log(_T("ImpersonationLevel: %s"), GetImpersonationLevel(stats.ImpersonationLevel));
...
void DumpAccessToken(CAccessToken& at)
{
	...
	if (!at.GetType(&type))
		...
		Log(_T("Type: %s"), GetTokenType(type));
	...
	if (type == TokenImpersonation)
	{
		...
		if (!at.GetImpersonationLevel(&sil))
			...
			Log(_T("Impersonation Level: %s"), GetImpersonationLevel(sil));
	}
...
#endif
{
	...
	{
		Log(_T("Security Descriptor for object %s:"), g_aObjects[i]);
		switch (g_eot)
		{
