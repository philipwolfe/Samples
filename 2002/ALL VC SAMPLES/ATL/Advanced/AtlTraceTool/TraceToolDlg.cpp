...
void CTraceToolDlg::PopulateTree()
{
	...
	for( int iProcess = 0; iProcess < aidProcesses.GetSize(); iProcess++ )
	{
		...
		if( dwProcess != 0 )
		{
			...
			TCHAR szText[256];
#ifdef _UNICODE
			_stprintf( szText, _T( "%s:0x%08x" ), info.szName, idProcess );
#else
			_stprintf( szText, _T( "%S:0x%08x" ), info.szName, idProcess );
#endif
			...
			ATLTRY(pPS = new PROCESSSETTINGS);			
			pPS->bModified = FALSE;
...
#ifdef _UNICODE
				_stprintf( szText, _T( "%s" ), module.szName );
#else
				_stprintf( szText, _T( "%S" ), module.szName );
#endif
				HTREEITEM hModuleItem = m_tree.InsertItem( szText, hProcessItem );
				DWORD_PTR dwModule = module.dwModule;

				TRACESETTINGS *pTS = NULL;
				ATLTRY(pTS = new TRACESETTINGS);			
				pTS->bModified = FALSE;
...
#ifdef _UNICODE
					_stprintf( szText, _T( "%s" ), category.szName );
#else
					_stprintf( szText, _T( "%S" ), category.szName );
#endif
					HTREEITEM hCategoryItem = m_tree.InsertItem( szText, hModuleItem );

					TRACESETTINGS *pTS2 = NULL;
					ATLTRY(pTS2 = new TRACESETTINGS);			
...
void CTraceToolDlg::OnBnClickedSave()
{
	...
	if(hItem)
	{
		...
		if (IDOK == dlg.DoModal())
			AtlTraceSaveSettings(dlg.GetPathName(), pPS->dwProcess);
	}
}
