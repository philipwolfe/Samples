...
END_PROPSET_MAP()
				...
				const VARIANT* rgRestrictions)
	{
		...
		// The user didn't specify a TABLE_NAME restriction
		if (bNameSet == false)
			_tcscpy(szFile, _T("C:\\public\\DataFile.dat")); // modify the path to point to your default data file
		...
		*pcRowsAffected = 5;		// for the five columns
		...
		_tcscpy(&szDir[0], _T("c:\\public\\DataFile.dat")); // modify the path to point to your default data file
...
END_PROPSET_MAP()
	...
	HRESULT Execute(DBROWCOUNT* pcRowsAffected, ULONG, const VARIANT*)
	{
		...
		lstrcpyW(trData[0].m_szName, OLESTR("String"));
		...
		lstrcpyW(trData[1].m_szName, OLESTR("Long"));
