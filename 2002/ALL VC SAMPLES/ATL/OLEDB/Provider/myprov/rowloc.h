...
public:
		...
		DBROWCOUNT cRows, DBCOUNTITEM * pcRowsObtained, HROW ** prghRows)
	{
		...
		if ((cbBookmark == 1) && (*pBookmark == DBBMK_LAST))
			pT->m_iRowset = (DBROWOFFSET)pT->m_rgRowData.GetCount() + 1;
...
//      (lRowsOffset >= 0) ? m_iRowset -= 1 : m_iRowset += 1;
		if (pT->m_iRowset < 0 || pT->m_iRowset > (DBROWOFFSET)pT->m_rgRowData.GetCount())
		{
		...
		HROW rghRows[], DBROWSTATUS rgRowStatus[])
	{
		...
		for (ULONG l=0; l<cRows; l++)
		{
			...
			if (pT->CreateRow((DBROWOFFSET)*rgpBookmarks[l], ulRowsObtained, &rghRows[l]) != S_OK)
			{
				bErrors = TRUE;
			}
			else
	...
	HRESULT ValidateBookmark(DBBKMARK cbBookmark, const BYTE* pBookmark)
	{
		...
		size_t nRows = pT->m_rgRowData.GetCount();
