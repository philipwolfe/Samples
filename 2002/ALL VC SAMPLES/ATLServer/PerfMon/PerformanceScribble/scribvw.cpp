...
#include "PerfDll\PerformanceManager.h"
...
CScribbleView::CScribbleView()
{
	...
	perfMgr.Initialize();
	{
		CPerfLock lock(&perfMgr);
		HRESULT hr = lock.GetStatus();
		if (SUCCEEDED(hr))
			perfMgr.CreateInstanceByName(1, L"ScribbleMouse", reinterpret_cast<CPerfObject**>(&perfObj));
		else
			perfObj = 0;
	}
...
//	ATLTRACE("%d %d\n", point.x, point.y );
	...
	if (perfObj)
	{
		// use interlocked API for thread safety, because the counters can be inspected
		// at any time from another thread.
		InterlockedExchangeAdd( (LPLONG)&perfObj->xaxis, abs(point.x - last.x));
		InterlockedExchangeAdd( (LPLONG)&perfObj->yaxis, abs(point.y - last.y));

		eventCount++;
		DWORD timeNow = timeGetTime();
		if (timeNow >= lastTime + 1000)
		{
			perfObj->eventsPerSec = (DWORD)((float)eventCount / ((float)(timeNow - lastTime)/1000.0f));
			ATLTRACE("%d\n", perfObj->eventsPerSec);
			eventCount = 0;
			lastTime = timeNow;
		}
		last = point;
