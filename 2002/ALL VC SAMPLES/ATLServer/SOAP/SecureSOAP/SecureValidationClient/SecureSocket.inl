...
inline bool CSecureEvtSyncSocket::Create(WORD wFlags)
{
...
// HTTPS modified
inline bool CSecureEvtSyncSocket::Connect(const SOCKADDR* psa) 
{
	if (m_bConnected)
	...
	*pdwSize	=	0;
	for( iIndex = 0; iIndex < nCount; iIndex ++ )
	{
		...
		*pdwSize	+=	dwMessageSize;
		...
		memcpy( pbIoBuffer + Sizes.cbHeader, pBuffers[iIndex].buf, dwMessageSize );
...
inline bool CSecureEvtSyncSocket::Read(const unsigned char *pBuff, DWORD *pdwSize) 
{
	...
	while(TRUE)
    {
        ...
        if(pDataBuffer)
        {
			...
			memcpy( (void*)(pBuff + dwActualReadSize), (PBYTE)pDataBuffer->pvBuffer, pDataBuffer->cbBuffer);
