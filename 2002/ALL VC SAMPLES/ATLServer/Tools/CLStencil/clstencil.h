...
//		delete this;
		...
		LPCSTR szVerb) throw()
	{
		...
		if (szQueryString == NULL)
			*m_szQueryString = 0;
			...
			strcpy(m_szQueryString, szQueryString);

		if (szContentType == NULL)
			*m_szContentType = 0;
		else
			strcpy(m_szContentType, szContentType);

		if (szVerb == NULL)
			strcpy(m_szVerb, "GET");
		else
			strcpy(m_szVerb, szVerb);
	...
	void SetPathTranslated(LPCSTR szPath)
	{
		...
		memcpy(m_szScriptPathTranslated, m_szPathTranslated, nResult);
...
//		LogServerError("IHttpServerContext::GetContentType not supported in command line mode.\r\n");
		...
		DWORD * pdwSize) throw()
	{
		...
		LPCSTR	szVar	=	getenv(szVariableName);
		
		if ( szVar )
		{
			if( pvBuffer && *pdwSize  > strlen(szVar) )
				strcpy( pvBuffer, szVar );
			else
			{
...
public:
		...
		LPCSTR szVerb)
	{
		...
		ServerContext.SetPathTranslated(szFile);
