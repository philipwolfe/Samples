...
void CAtlGdiWorld::SpecialReg(BOOL bRegister)
{
	...
	if (bRegister)
	{
		...
		hr = pICatRegister->RegisterClassImplCategories(    __uuidof(CAtlGdiWorld),
															cIDs, IDs ) ;
		ASSERT_HRESULT(hr) ;
	}
	else
	{
		...
		hr = pICatRegister->UnRegisterClassImplCategories(  __uuidof(CAtlGdiWorld),
															cIDs, IDs);
		ASSERT_HRESULT(hr) ;
...
HRESULT CAtlGdiWorld::CreateVisualForModel(IATLTangramModel* pModel)
{
	...
	HRESULT hr = ::CoCreateInstance(    __uuidof(CAtlTangramGdiVisual),
										NULL,
										CLSCTX_INPROC_SERVER,
										__uuidof(IAtlTangramGdiVisual),
										(void**)&pVisual) ;
	if (FAILED(hr))
	{
