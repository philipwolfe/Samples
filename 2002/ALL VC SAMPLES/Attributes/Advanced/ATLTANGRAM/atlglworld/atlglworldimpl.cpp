...
#import "..\atltangramcanvas\_atltangramcanvas.tlb" 

using namespace ATLTANGRAMCANVASAttrib;
...
void CAtlGLWorld::SpecialReg(BOOL bRegister)
{
	...
	if (bRegister)
	{
		...
		hr = pICatRegister->RegisterClassImplCategories(    __uuidof(CAtlGLWorld),
															cIDs, IDs ) ;
		ASSERT_HRESULT(hr) ;
	}
	else
	{
		...
		hr = pICatRegister->UnRegisterClassImplCategories(  __uuidof(CAtlGLWorld),
															cIDs, IDs);
		ASSERT_HRESULT(hr) ;
...
HRESULT CAtlGLWorld::Initialize(HWND hWnd, double modelWidth, double modelHeight)
{
	...
	HRESULT hr = ::CoCreateInstance(__uuidof(ATLTANGRAMCANVASAttrib::CAtlTangramCanvas),
	...
	hr = m_pCanvasUnknown->QueryInterface(__uuidof(ATLTANGRAMCANVASAttrib::IAtlTangramCanvas), (void**)&m_pCanvas) ;
	if (FAILED(hr))
	{
									...
									IUnknown** ppITangramVisual)
{
	...
	*ppITangramVisual = NULL ;
	...
	if (0 <= index && index < (int)m_VisualList.size())
	{
#pragma warning(disable:4786)
...
HRESULT CAtlGLWorld::CreateVisualForModel(IATLTangramModel* pModel)
{
	...
	HRESULT hr = ::CoCreateInstance(__uuidof(CAtlTangramGLVisual),
									NULL,
									CLSCTX_INPROC_SERVER,
									__uuidof(IAtlTangramGLVisual),
									(void**)&pVisual) ;
	if (FAILED(hr))
	{
...
// ModelToDevice
//
HRESULT CAtlGLWorld::ModelToDevice(TangramPoint2d ptIn, TangramPoint2d* pptOut)
{
	//Precondition
...
int CAtlGLWorld::Pick(POINT pt)
{
	...
	m_bPicking = FALSE ;
	if ( (NameHit > 0) && (NameHit <= (int)m_VisualList.size()) )
	{
		return --NameHit;
	}
	else
