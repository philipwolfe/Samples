...
chdir "Aggreg"; `nmake /f Aggreg.mak CFG="Win32 $config"`; chdir "..";

$msg = `nmake /f COMMap.mak cfg="Win32 $config"`;
print RESULT "$msg\n";
if ($? != 0) {
...
chdir "$config";
if($type =~ /dll/i){
	$command = "regsvr32 /c /s COMMap.dll";
}
elsif ($type =~ /exe/i){
	$command = "COMMap.exe /regserver";
}
else {
	$command = "COMMap.exe";
}
