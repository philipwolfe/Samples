...
void* CChkBookDoc::OnCreateNewRecord(int nNewRecordIndex)
{
	...
	DWORD dwCents = 0L;
	CString strDate(CTime::GetCurrentTime().Format("%m/%d/%Y"));
	CString strPayTo;
	CString strMemo;
...
BOOL CChkBookDoc::UpdateCheck(CView* pSourceView, UINT nCheckNo,
		DWORD dwCents, LPCTSTR lpszPayTo, LPCTSTR lpszDate,
		LPCTSTR lpszMemo)
{
	// Check the date
	COleDateTime date;
	if (date.ParseDateTime(lpszDate, VAR_DATEVALUEONLY) == FALSE) {
