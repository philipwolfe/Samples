...
BOOL CCheckView::MaybeCommitDirtyCheck()
{
	...
	{
	case IDYES:
		if (pDoc->UpdateCheck(this, m_nCheckNo, m_dwCents,
			m_strPayTo, m_strDate, m_strMemo) == FALSE)
		{
			SelectDate();
			return(FALSE);
		}
...
void CCheckView::OnEditCommitCheck()
{
	...
	// this check.
	if (GetDocument()->UpdateCheck(this, m_nCheckNo, m_dwCents, m_strPayTo,
		m_strDate, m_strMemo) == FALSE)
	{
		SelectDate();	
	}
}
