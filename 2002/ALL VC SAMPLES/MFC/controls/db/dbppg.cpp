...
void CDbPropPage::OnSelchangeDatasource()
{
	...
	strncpy((LPSTR) szSource, m_dataSource, m_dataSource.GetLength());
	...
	if (retcode == SQL_SUCCESS)
	{
		...
		if (retcode == SQL_SUCCESS)
		{
			...
			if (GetDriverLevel(hdbc) == 2)
			{
				...
				cbBrowse = STR_LEN;
				strncpy((LPSTR) szSourceInfo, "DSN=", 4);
				strncat((LPSTR) szSourceInfo, m_dataSource, m_dataSource.GetLength());
...
void CDbPropPage::OnSelchangeTables()
{
	...
	{
		*pstrTrim = '\0';
		strncpy((LPSTR) szSelectedOwner, (LPSTR) szSelectedTable, 256);
		strncpy((LPSTR) szSelectedTable, pstrTrim+1, 256+1);
		bHasOwner = TRUE;
	}
	else
	{
