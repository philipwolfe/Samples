...
#endif
{
	...
	if (pszDllEntryPoint != NULL) {
		ptszDllEntryPoint = (TCHAR *)_alloca((1 + strlen(pszDllEntryPoint)) * (sizeof(TCHAR)));
#ifdef _UNICODE
		wsprintf(ptszDllEntryPoint, _T("%hs"), pszDllEntryPoint);
#else // !_UNICODE
		strcpy(ptszDllEntryPoint, pszDllEntryPoint);
#endif
	}
