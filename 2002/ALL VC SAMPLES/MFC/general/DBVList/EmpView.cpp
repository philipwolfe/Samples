...
void CEmpView::GetDispInfo(LVITEM* pItem)
{
	...
	if(pItem->mask & LVIF_TEXT)
	{
		...
		if(!pSet->IsEOF())
		{
			...
			{
			case 0:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Email);
				break;
			case 1:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Name);
				break;
			case 2:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Phone);
				break;
			case 3:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Location);
				break;
			case 4:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Title);
				break;
			case 5:
				lstrcpy(pItem->pszText, (LPCTSTR)pSet->m_Department);
				break;
			}
		}
	}
...
void CEmpView::UpdateFilter(CString strCurQuery)
{
	...
	GetListCtrl().SetItemCountEx(pSet->GetRecordCount());
