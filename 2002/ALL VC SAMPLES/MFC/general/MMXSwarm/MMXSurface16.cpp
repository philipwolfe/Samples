...
void CMMXSurface16Intrinsic::AdjustWidth(int *pWidth)
{
	...
	// Then, our x-border needs to be 4 pixels wide as well.
	int nVisibleWidth = *pWidth - m_kDeltaX;
	nVisibleWidth = (nVisibleWidth + 3) & ~3; // round up by 4
	nVisibleWidth += (4 - m_kDeltaX);
	*pWidth = nVisibleWidth;
}
...
void CMMXSurface16Intrinsic::OnCreated()
{
    ...
    m_delta = int((GetPixelAddress(0,1) - GetPixelAddress(width,0))/8);
}
