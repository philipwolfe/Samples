...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	...
	wsprintf(szBuffer, szFormat, hwnd, szTemp);
...
void CWndListDlg::OnSelChangeWndList()
{
	...
	if (iSel >= 0)
	{
		...
		if( _stscanf(szBuffer, szParse, &hwnd) == 1 )
	...
	if (bValid)
	{
		...
		wsprintf(szBuffer, szFormat, hwndParent, szTemp);
		GetDlgItem(IDC_PARENT)->SetWindowText(szBuffer);

		RECT rect;
		::GetWindowRect(hwnd, &rect);
		wsprintf(szBuffer, _T("(%d,%d) - (%d,%d)"), rect.left, rect.top,
				rect.right, rect.bottom);
		GetDlgItem(IDC_RECT)->SetWindowText(szBuffer);

		LONG lStyle = ::GetWindowLong(hwnd, GWL_STYLE);
		wsprintf(szBuffer, _T("%08lX"), lStyle);
