...
{
	ASSERT(pDC != NULL);
	ASSERT(pDC->GetMapMode() == MM_SPIRO);
	...
	for (int i = nFirst; i <= m_nTotalDrawn; i++)
	{
		...
		case LINE_TO:
			...
			else
			{
				...
				{
					ASSERT(ptLast.x != -5000 && ptLast.y != 5000);  // there should be a MOVE_TO before
	...
	{
		INT_PTR nLast = m_arrCommands.GetUpperBound();
		int nPasses = 0;

		for (int i = 0; i <= nLast && nPasses < 2; i++)
		{
			figCommand = m_arrCommands.GetAt(i);
...
void CSpiroDoc::DeleteContents() 
{
	...
	if (pos != NULL)  // on shut down the view has been destroyed already
	{
		...
		ASSERT(pView != NULL && pView->IsKindOf(RUNTIME_CLASS(CSpiroView)));
