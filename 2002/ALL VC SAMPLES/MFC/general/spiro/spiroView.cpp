...
void CSpiroView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bDroppingAnchor || m_bDroppingWheel)
	{
		...
		if (m_bDroppingAnchor)
		{
			...
			ASSERT(pRingDrop != NULL);  
			m_bDroppingAnchor = false;
			ASSERT(m_pAnchor == NULL);
			m_pAnchor = pRingDrop;
...
{
	ASSERT(ppPiece != NULL && *ppPiece != NULL);
	CRgn	rgnPiece;
...
{
	ASSERT(m_pWheel != NULL && m_pAnchor != NULL);
	...
	for (iCycle = 0;iCycle < 200;iCycle++)
	{
		...
		for (i = 0; i < iCycle; i++)
		{
			...
			if (rgdCycleLimits[0][rgCycleOrder[i]] > rgdCycleLimits[0][iCycle])
			{
				...
				memmove((int*)rgCycleOrder + i + 1, (int*)rgCycleOrder + i, (iCycle - i) * sizeof(int));
...
void CALLBACK  CSpiroView::TimerCallBack(HWND , UINT , UINT , DWORD )
{
	ASSERT(g_pView != NULL && g_pView->IsKindOf(RUNTIME_CLASS(CSpiroView)));
...
void CSpiroView::PressButton(UINT id)
{
	...
	ASSERT(m_pWheel != NULL);     // otherwise the buttons should be disabled.
...
{
	CSpiroDoc* pDoc = GetDocument();
	ASSERT(pDoc != NULL && pDoc->IsKindOf(RUNTIME_CLASS(CSpiroDoc)));
