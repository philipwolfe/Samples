	...
	LPCTSTR pstrFullPath, LPCTSTR pstrContentType)
{
	...
	if (hFile == INVALID_HANDLE_VALUE)
	{
		...
		*pCtxt << "File open failed on ";
		*pCtxt << pstrFullPath << "!  Error was ";
		*pCtxt << long(dwError);
		*pCtxt << "<br>";

		*pCtxt << "Did you correctly copy all the graphics files, too?<br>";
	}
	else
	{
		...
		if (!bResult)
		{
			...
			DWORD dwError = GetLastError();
			*pCtxt << "TransmitFile called failed!  Error was ";
			*pCtxt << long(dwError) << "<br>";
