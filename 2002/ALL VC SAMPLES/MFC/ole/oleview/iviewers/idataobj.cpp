...
{
	CIDataObjectDlg * pIDOD =(CIDataObjectDlg *)GetWindowLongPtr( hDlg, DWL_USER ) ;
	...
	switch (uiMsg)
	{
		case WM_INITDIALOG:
			...
			SetWindowLongPtr( hDlg, DWL_USER, (LONG_PTR)pIDOD ) ;
			pIDOD->m_hDlg = hDlg ;
...
BOOL CIDataObjectDlg::DoIEnumFormatEtc( UINT nLevel, LPENUMFORMATETC pIFormatEtc )
{
	...
	while (pIFormatEtc->Next( 1, &fetc, &ulFE ) == S_OK)
	{
		...
		*pfetc = fetc ;
		...
		TCHAR szTemp[32] ;
		wsprintf( szTemp, _T( "%04lX:%04lX\t" ), (DWORD)HIWORD( (DWORD_PTR)fetc.ptd ), (DWORD)LOWORD( (DWORD_PTR)fetc.ptd ) ) ;
		lstrcat( sz, szTemp ) ;
...
LRESULT EXPORT CALLBACK fnEditSubclass( HWND hwnd, UINT uiMsg, WPARAM wP, LPARAM lP )
{
	if (uiMsg == WM_PAINT)
	{
		...
		CIDataObjectDlg * pIDOD =(CIDataObjectDlg *)GetWindowLongPtr( hwndParent, DWL_USER ) ;
