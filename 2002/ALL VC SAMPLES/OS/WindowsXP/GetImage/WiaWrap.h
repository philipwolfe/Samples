...
/*++
    ...
    CPropSpec is a wrapper class for the PROPSPEC struct
...
Methods
    ...
    CPropSpec(LPOLESTR _lpwstr)
        Initializes the struct to hold the string name of a property.

    CPropSpec(PROPID _propid)
        Initializes the struct to hold the PROPID of a property.
...
/*++
    ...
    CPropVariant is a wrapper class for the PROPVARIANT struct.
...
Methods
    ...
    ~CPropVariant()
        Frees all elements that can be freed in the struct.

    CPropVariant(const CPropVariant &rhs)
    CPropVariant &operator =(const CPropVariant &rhs)
        Copies the contents of one CPropVariant to another.

    CPropVariant(LONG Value)
    CPropVariant &operator =(LONG Value)
        Initializes the struct to hold a LONG value.

    CPropVariant(REFCLSID Value)
    CPropVariant &operator =(REFCLSID Value)
        Initializes the struct to hold a GUID value.
...
Parameters
    ...
    PropSpec
        [in] Reference to a CPropSpec structure that specifies which 
...
/*++
    ...
    The WriteProperty function writes a value to a WIA property storage
...
Parameters
    ...
    PropVariant
        [in] Reference to a CPropVariant structure that contains the value 

    propidNameFirst
        [in] Specifies the minimum value for the property IDs that the 
        method must assign if the PropSpec parameter specifies a string-named 
        property for which no property IDs currently exist. If the
        string-named property specified already exists in this set, and 
        thus already have a property ID, this value is ignored. 


Return Values
    
    WriteProperty returns S_OK on success or a standard COM error 
    if it fails for any reason.
