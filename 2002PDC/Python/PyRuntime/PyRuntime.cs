...
public class PyStringType : PyType, IPySequence {
	...
	PyObject PyString_Split(PyObject self, PyObject[] args, PyObject kw)
	{
		...
		return Converters.PyList_FromCollection(Array.AsList(a));
	}
	public static
	...
	void tp_print(PyObject self, System.IO.TextWriter s, int flags) {
		...
		for (int i = 0; i < ss.Length; i++) {
			...
			if (c == quote || c == '\\') {
				...
				s.Write("\\");
				s.Write(System.Radix.ToString(c & 0377, 8));
			} else
				s.Write(c);
		}
		s.Write(quote);
	...
	PyObject tp_repr(PyObject self) {
		...
		for (int i = 0; i < ss.Length; i++) {
			...
			if (c == quote || c == '\\') {
				...
				// XXX - rethink in terms of Unicode!
				s.Append("\\");
				s.Append(System.Radix.ToString(c & 0377, 8));
			} else
				s.Append(c);
		}
		s.Append(quote);
...
public class PyFloatType : PyType, IPyNumber {
	...
	AsStringEx(PyObject v, int precision)
	{
		...
		String ret = Double.Format((double)v.ob, null);
		...
		for (; offset < ret_len; offset++) {
			...
			if (System.Globalization.CharacterInfo.IsDigit(ret[offset]))
				break;
		}
		if (offset==ret_len) {
			ret = ret + ".0";
...
//			throw new ValueError("list.remove(x): x not in list");
	...
	PyList_Count(PyObject self, PyObject[] args, PyObject kw) {
		...
		int cnt = 0;
		int end = op.Count;
		while ( (index=op.IndexOf(val, index+1, end)) !=-1)
			cnt++;
...
public class PyClassType : PyType {
	...
	public override PyObject tp_getattr(PyObject s, System.String name)
	{
		...
		try {
			PropertyInfo pi = self.GetProperty(name, BindingFlags.Default | BindingFlags.Static);
			if (pi != null) {
				// This object does have a property of that name - use it.
...
***/
	...
	public override PyObject tp_call(PyObject s, PyObject[] args, PyObject kw)
	{
		...
		if (numArgs < neededArgs) {
			...
			flags |= BindingFlags.DefaultValueFull;
		}
...
#endif
		BindingFlags flags = BindingFlags.InvokeMethod | BindingFlags.Default | BindingFlags.DefaultValueBinding | BindingFlags.DefaultValueFull;
		object ret;
		try {
			if (self.mi==null) 
...
//	void 	tp_print(PyObject , System.IO.TextWriter *, int);
	...
	PyObject	tp_getattr(PyObject s, String this_name) {
		...
		int base_len = self.Name.Length - System.IO.File.GetExtension(self.Name).Length;
...
//		Type mainType = mod.GetType("builtins$main");
		Assembly ass = Assembly.Load("Python.Builtins.Builtins");
		if (ass==null)
