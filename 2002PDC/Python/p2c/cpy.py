# The Managed Python compiler.
# Copyright 1997-1999 Greg Stein and Bill Tutt
# Portions written by Mark Hammond are
...
def usage(extra = None):
    if extra:
        print "***", extra
        print
    print "cpy.py [/opt:val ...] source_file ..."
    print "Options:"
...
def find_and_set_homes():
    """Set some useful environment variables that may be referenced by the user
    (and expanded as compiler options or paths are set)
    """
    if not os.environ.has_key("PYTHON_HOME"):
        # Use the Python OS module as a landmark.
        py_home = os.path.join(os.path.split(os.__file__)[0], "..")
        os.environ["PYTHON_HOME"] = os.path.abspath(py_home)

    if not os.environ.has_key("COMPILER_HOME"):
        os.environ["COMPILER_HOME"] = os.path.abspath( os.path.split(sys.argv[0])[0] )
    ...
    for arg in sys.argv[1:]:
        if arg[0] in '-/':
            ...
            if opts.print_tree:
                ...
                # Do the real compile.
                err_no = compile_code(compiler, os.path.expandvars(arg))
                if err_no:
                    compiler.finalize()
                    return err_no
