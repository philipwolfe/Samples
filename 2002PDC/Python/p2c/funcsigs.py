    ...
    if func.is_ctor:
        ...
        for base in cctx.all_cor_bases:
            ...
            check_info = lambda info, name=func.name: \
                                     glue.MethodInfo_GetName(info) == name and \
                                     glue.MethodBase_Attributes(info) & constants.MethodAttributes_Virtual == constants.MethodAttributes_Virtual
            new_infos = filter(check_info, new_infos)
#            print "base", glue.Type_GetFullName(base), "found", new_infos
    ...
    if len(infos)>0:
        ...
        if func.varargs:
            check_info = lambda info, nargs=num_positional_args: \
                                     len(glue.MethodBase_GetParameters(info))>=nargs
        else:
            check_info = lambda info, nargs=num_positional_args: \
                                     len(glue.MethodBase_GetParameters(info))==nargs
    ...
    for info in infos:
        ...
        if func.is_ctor:
            ...
            ret_type = glue.MethodInfo_GetReturnType(info)
        attr = glue.MethodBase_Attributes(info)
...
class PyMethodSignatureManager(MethodSignatureManager):
    def PrepareContext(self):
        ...
        if defaults:
            ...
            self.fb_args = parent_ctx.typeBuilder.DefineField(array_name, array_type, constants.FieldAttributes_Private | constants.FieldAttributes_Static)
            emitters = []
    ...
    def _EmitPrivateCtor(self):
        ...
        def_ctor_info = cctx.typeBuilder.DefineConstructor(attr, 0, ())
    ...
    def EmitFunctionObject(self, method_info):
        ...
        ctor_info = glue.Type_GetConstructor(delegate_type, (getCORType("System.Object"), getCORType("System.Int32")))
...
class CORMethodSignatureManager(MethodSignatureManager):
    def EmitFuncHeader(self):
        ...
        param_types = map(glue.Type_GetFullName, param_types)
        ...
        for i, name, param_typ in map(None, range(len(arg_names)), arg_names, param_types):
            ...
            have_default = 0
            if default_index >= 0:
                param_attr = param_attr | constants.ParameterAttributes_Optional
            ...
            pi = fctx.methBuilder.DefineParameter(i+1, param_attr, name)
            if have_default:
    ...
    def EmitThunk(self, method_info):
        ...
        for t in param_types:
            type_name = glue.Type_GetFullName(t)
        ...
        ret_type_name = glue.Type_GetFullName(ret_type)
