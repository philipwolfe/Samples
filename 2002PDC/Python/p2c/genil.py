...
def LoadCOMPlusTypeLib():
    ...
    try:
        tlb = pythoncom.LoadTypeLib("mscorlib.tlb")
    except pythoncom.com_error, details:
        print "ERROR:  Can not load the COM+ type library"
        print details
        sys.exit(1)
    try:
        # This was a late addition to the COM support!
        doer = gencache.EnsureModuleForTypelibInterface
    except AttributeError:
        doer = None
    if doer is not None:
        return doer(tlb, bForDemand = 1)
...
def format_method_info(info):
    param_infos = glue.MethodBase_GetParameters(info)
    param_infos = map(lambda info : glue.Type_GetFullName(info.ParameterType), param_infos)
    param_sig = string.join(param_infos, ", ")
    name = glue.MethodInfo_GetName(info)
    try:
        ret_type = glue.Type_GetFullName(glue.MethodInfo_GetReturnType(info))
...
def find_best_methods(name, infos, input_arg_types, only_static = 0, exact = 0):
    ...
    if name is not None:
        infos = filter(lambda info, name=name: glue.MethodInfo_GetName(info) == name, infos)

    if trace_results:
        print "Matching function '%s' for types %s:" % (name, map(glue.Type_GetFullName,input_arg_types))
    ...
    # Filter static
    if only_static:
        flt = lambda info: glue.MethodBase_IsStatic(info)
    else:
        flt = lambda info: not glue.MethodBase_IsStatic(info)
    ...
    for info in infos:
        pis = glue.MethodBase_GetParameters(info)
        ...
        for pi, at in map(None, pis_look, input_arg_types):
            pt = pi.ParameterType
            ptname = glue.Type_GetFullName(pt)

            if not glue.Type_IsArray(pt) and \
    ...
    def getCORType(self, typestring, really_need_it = 1):
        ...
        if self.extra_assemblies is None:
            ...
            if not self.options.no_python_runtime:
                try:
                    self.extra_assemblies.append(glue.Assembly_Load("Python.dll"))
...
#    else:
    ...
    def makeModuleContext(self, filename):
        ...
        mod = self.actx.ass_builder.DefineDynamicModule_4(module_name, self.options.output_file, self.options.debug_info)
...
#    assert mctx is not None, "where is the mctx!"
        ...
        self.ilgen = methBuilder.GetILGenerator()
    ...
    def _BoxValueType(self, type_name):
        ...
        if cor_type is None or not glue.Type_IsValueType(cor_type):
            return
        ...
        if type_to in T_PY_ALL:
            ...
            if type_from in T_PY_ALL:
                ...
                if type_to_ob is not None:
                    ...
                    if glue.Type_IsValueType(type_to_ob):
...
#        cor_args = map(self.compiler.getCORType, cor_args)
    ...
    def EmitPropertyGet(self, typ_name, propName):
        ...
        meth_info = glue.PropertyInfo_GetGetMethod(prop_info)
#        meth_info = prop_info.GetGetMethod()
    ...
    def EmitField(self, opcode, *args):
        ...
        self.ilwrap.EmitField(opcode, fi)
...
#                print "Sequence point at", node.lineno
        ...
        ret = fn(node)
        assert type(ret)==type(''), "node did not return a valid value: '%s'" % (node,)
        return ret
    ...
    def n_class(self, node):
        ...
        for base in base_classes:
            ...
            if this_base_type is not None:
                if glue.Type_IsClass(this_base_type):
                    if type_base_type is not None:
                        # Second class listed.
                        msg = "Class '%s' is using Python derivation from '%s': second or suqsequent base" % (class_name, glue.Type_GetFullName(this_base_type))
                        ...
                        if len(ctors)==0:
                            ...
                            msg = "Class '%s' is using Python derivation from '%s': ctor is protected" % (class_name, glue.Type_GetFullName(this_base_type))
                            self.compiler.verbose(1, msg, NoWhere)
                            this_base_type = None # Force into the Python class list
                        else:
                            msg = "Class '%s' is using COM+ derivation from '%s'" % (class_name, glue.Type_GetFullName(this_base_type))
                            self.compiler.verbose(1, msg, NoWhere)
                            type_base_type = this_base_type
                elif glue.Type_IsInterface(this_base_type):
...
##    il.EmitMethodCall("System.Type", "GetType", ("System.String",))
    ...
    def n_from(self, node):
        ...
        for name in items:
            ...
            self.name_assign(name)
        il.Emit(Opcodes.Pop)
        return T_VOID

    def n_import(self, node):
        for mod_name in node[1]:
            base_name = string.split(mod_name, ".")[0]
            # emit the module object
            self.il.pushConstant(mod_name)
            self.emitAbstractCall("PyImport_ImportModule")
            # and assignment
            self.name_assign(base_name)
        return T_VOID
...
#    print "n_getattr", node
    ...
    def _EmitArgsCOMCall(self, info, args, star_args, dstar_args):
        ...
        param_types = glue.MethodBase_GetParameters(info)
...
#        assert len(args) <= len(param_types), "Too many args for this function - expected %d, got %d!" % (len(param_types), len(args))
        for arg, typ in map(None, args, param_types[:len(args)]):
            ...
            self.dispatch_and_convert(arg, glue.Type_GetFullName(typ))
        # Emit any default values.
        if len(param_types) - len(args) > 0:
            pis = glue.MethodBase_GetParameters(info)[len(args):]
            for pi in pis[len(args):]:
                assert pi.IsOptional, "Setting default value for param, but its not optional!"
                def_val = pi.DefaultValue
                il.pushConstant(def_val, glue.Type_GetFullName(pi.ParameterType))
            ...
            if cor_base_type is not None and \
                ...
                if tail=='__init__':
                    ...
                    if not ctor_infos:
                        ...
                        if len(ctor_infos)>1:
                            param_infos = glue.MethodBase_GetParameters(ctor_infos[0])
                            param_infos = map(lambda info : glue.Type_GetFullName(pi.ParameterType), param_infos)
                            sig = string.join(param_infos, ", ")
                            self.compiler.warning(2, "%d possible constructor matches - using %s(%s)" % (len(ctor_infos), glue.Type_GetFullName(cor_base_type),sig))
...
#                    tok, typ = self.fctx.getDeclaredLocal(self.fctx.self_name)
                    ...
                    if tail == self.fctx.name: # We are calling _our_ base implementation.
                        ...
                        if len(poss_infos)==1:
                            ...
                            params = glue.MethodBase_GetParameters(method_info)
                    ...
                    for arg_node, to_type_ob in map(None, args, base_types):
                        to_type = glue.Type_GetFullName(to_type_ob)
                        self.dispatch_and_convert(arg_node, to_type)
                    ret_type = glue.MethodInfo_GetReturnType(method_info)
                    ret_type = glue.Type_GetFullName(ret_type)
                    # And make the call.
        ...
        if cor_type is not None and tail:
            ...
            if cor_type is not None:
                ...
                poss_infos = glue.Type_GetMethods(cor_type)
                poss_infos = filter( lambda info: glue.MethodBase_IsStatic(info), poss_infos)
                poss_infos = filter(lambda info: glue.MethodInfo_GetName(info) == "__init__", poss_infos)
                if not poss_infos:
        ...
        if method_info:
            if is_py_sig:
                ...
                if is_ctor:
                    ...
                    if glue.MethodInfo_GetName(method_info) == "PyObject_Call":
                ...
                if is_ctor:
                    ...
                    if glue.MethodBase_IsVirtual(method_info):
                        opcode = Opcodes.Callvirt
                    else:
                        opcode = Opcodes.Call
                    self.il.EmitMethod(opcode, method_info)
                    return glue.Type_GetFullName(glue.MethodInfo_GetReturnType(method_info))
...
#        assembly_access = constants.AssemblyBuilderAccess_Run
        ...
        output_file = compiler.assembly_output_filename
        options = compiler.options
        app_domain = glue.Thread_GetDomain()
        ass_name = glue.CreateAssemblyName()
        ass_name.Name = options.assembly_name or os.path.splitext(output_file)[0]
        ass_name.Title = options.assembly_title or output_file
...
#        self.ass_builder = app_domain.DefineDynamicAssembly(ass_name, assembly_access) # Broke in 1626

    def finalize(self):
        try:
            self.ass_builder.Save(self.compiler.assembly_output_filename)
        except pythoncom.com_error, (hr, msg, exc, arg):
            if exc:
                hr = exc[0] or exc[-1]
                msg = exc[2] or msg
...
#    guid_null = pythoncom.IID_NULL
        ...
        self.typeBuilder = self.mod.DefineType_4(module_name + "$main", constants.TypeAttributes_Public)
...
class ClassContext(Context):
    ...
    def __init__(self, compiler, mctx, name, base_class, base_interfaces):
        ...
        self.typeBuilder = mctx.mod.DefineType_2(name, constants.TypeAttributes_Public, base_class, base_interfaces)
#    print "base class is", `base_class`, glue.Type_GetFullName(base_class)
        self.name = name
        self.seen_ctor = 0
        ...
        self.fb_dict = self.typeBuilder.DefineField("__class_dict__", self.type_dict, constants.FieldAttributes_Public | constants.FieldAttributes_Static)

        self.fb_bases = self.typeBuilder.DefineField("__bases__", compiler.getCORType(T_PYOBJECT), constants.FieldAttributes_Public | constants.FieldAttributes_Static)
        ...
        self.fb_inst_dict = self.typeBuilder.DefineField("__dict__", self.type_dict, constants.FieldAttributes_Public)
...
#    ctor_infos = filter(lambda info, base=base: glue.Type_Equals(base, :
        for ctor_info in ctor_infos:
            param_types = glue.MethodBase_GetParameters(ctor_info)
            param_types = map(lambda info: info.ParameterType, param_types)
            attr = glue.MethodBase_Attributes(ctor_info)
...
#            methBuilder = self.typeBuilder.DefineConstructor(attr, 0, param_types)
            ...
            pis = glue.MethodBase_GetParameters(ctor_info)
...
#      il.BeginCatchBlock(self.compiler.getCORType("Python.Builtins.exceptions.KeyError"))
            ...
            for prop_name in self.registered_properties.keys():
                pb = self.typeBuilder.DefineProperty(prop_name, prop_attr, obj_type, (), None)
                # The Getter
                propMethBuilder = self.typeBuilder.DefineMethod("get_" + prop_name, method_attributes, obj_type, ())
...
class FunctionContextBase(Context):
    ...
    def __init__(self, compiler, mctx, cctx, name, descriptor):
        ...
        self.return_type_id = glue.Type_GetFullName(cor_ret_type)
        if self.return_type_id != T_VOID:
            ...
            if self.return_type_id == T_PYOBJECT:
                ...
                if glue.Type_IsValueType(cor_ret_type):
...
#        self.methBuilder = glue.TypeBuilder_DefineConstructor(builder, attr, descriptor)
