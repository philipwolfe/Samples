...
void AIBunny::doAIMove(void)
{
	...
	while (isActive()) {
		...
		{
			if(_pgm->dropBomb(_gameID, _userID))
...
bool AIBunny::validateAIBombDrop() {
	...
	if(_pgm->getNumberOfSoftwall(_gameID) > 0)
	{
		...
		if(
			((col > 0) && (_pgm->getCellObject(_gameID, row, col - 1) & SOFTWALL)) ||
			((row > 0) && (_pgm->getCellObject(_gameID,row - 1, col) & SOFTWALL)) ||
			((col < MAX_COL - 1) && (_pgm->getCellObject(_gameID,row, col + 1) & SOFTWALL)) ||
			((row < MAX_ROW - 1) && (_pgm->getCellObject(_gameID,row + 1, col) & SOFTWALL))
			)
			boolValidDrop = true;
	}
	...
	{
		int res = _pgm->getCellObject(_gameID, row, col + 1);
		if(
			// check if man is on left edge.
			((col == 0) && (!(_pgm->getCellObject(_gameID,row, col + 1) & HARDWALL))) ||
			// check if man is on upper edge.
			((row == 0) && (!(_pgm->getCellObject(_gameID,row + 1, col) & HARDWALL))) ||
			// check if man is on right edge.
			((col == (MAX_COL - 1)) && (!(_pgm->getCellObject(_gameID,row, col - 1) & HARDWALL))) ||
			// check if man is on the lower edge.
			((row == (MAX_ROW - 1)) && (!(_pgm->getCellObject(_gameID,row - 1, col) & HARDWALL))) ||
...
bool AIBunny::validateMove(Int32 row, Int32 col){
	...
	if((_pgm->getCellObject(_gameID,row, col) & BOMB) || (_pgm->getCellObject(_gameID,row, col) & HARDWALL) || (_pgm->getCellObject(_gameID,row, col) & SOFTWALL))
		return false;
...
bool AIBunny::canEscape(int row, int col, Int32 pathlen){
	...
	//if ((abs(getCurrentXLen() - 1) > 0)||(abs(getCurrentYLen())> 0)) {
		if (validateMove(row, col-1)) {
			...
			foundpath = canEscape(row, col-1, pathlen-1)&&((abs(getCurrentXLen())+ abs(getCurrentYLen())) >= pathlen);
			if (foundpath) return true;
			else EscapePath->pop_back();
		}
	...
	//Up
			...
			foundpath = canEscape(row-1, col, pathlen-1)&&((abs(getCurrentXLen())+ abs(getCurrentYLen())) >= pathlen);;
			if (foundpath) return true;
			else 	EscapePath->pop_back();
...
bool AIBunny::validateAIMove(BYTE row, BYTE col, BYTE ManRow, BYTE ManCol, bool *pboolSafeMove){
	...
	*pboolSafeMove = true;
	...
	if(_pgm->getCellObject(_gameID,row, col) & EXPLOSION)
	{
		*pboolSafeMove = false;
		return false;
	}
				...
				if(_pgm->svr->game[_gameID].bunny[m].bomb[b].range > abs(_pgm->svr->game[_gameID].bunny[m].bomb[b].c - col))
				{
					...
					if(_pgm->svr->game[_gameID].bunny[m].bomb[b].c < col)
					{
						...
						for(int c = _pgm->svr->game[_gameID].bunny[m].bomb[b].c + 1; c <= col; c++)
						{
							...
							if((_pgm->getCellObject(_gameID,row, c) & HARDWALL) || (_pgm->getCellObject(_gameID,row, c) & SOFTWALL))
							{
...

			{
				...
				if(_pgm->svr->game[_gameID].bunny[m].bomb[b].range >= abs(_pgm->svr->game[_gameID].bunny[m].bomb[b].r - row))
				{
					...
					
					{
						...
						{
							// if we find a wall, the bomb is blocked.
							if((_pgm->getCellObject(_gameID,r, col) & HARDWALL) || (_pgm->getCellObject(_gameID,r, col) & SOFTWALL))
							{
								boolBombBlocked = true;
								break;
							}
