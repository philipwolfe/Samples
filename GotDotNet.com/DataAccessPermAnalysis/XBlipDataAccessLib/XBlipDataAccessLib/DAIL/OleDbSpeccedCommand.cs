...
namespace XBLIP.DAIL.DataSources
{
	...
	internal class OleDbSpeccedCommand
	{
		...
		/// CDATA section begin
		/// </summary>
		private const string CDATA_BOUND_START = "<![CDATA[";

		/// <summary>
		/// CDATA section end
		/// </summary>
		private const string CDATA_BOUND_END = "]]>";
			...
			do
			{
				switch(readerCommandContent.getEntityName()) 
				{
					case "Param":
						...
						break;
					case "Return":
						setReturnMethod(readerCommandContent);
						readerCommandContent.moveToNextEntity();
						break;
					case "Text":
		...
		/// <summary>
		/// set the return method specced, this may either be a data set
		/// or a stored procedure return type (depends on the value of "type")
		/// </summary>
		/// <param name="readerNavigator">
		/// navigator pointing on the return value settings
		/// </param>
		private void setReturnMethod(XmlReaderEntityNavigator readerNavigator) 
		{
			string strType = readerNavigator.getEntityAttribute("type");

			bSpeccedReturnValue = true;

			if("recordSet" == strType)
				bReturnDataSet = true;
			else 
			{
				bReturnDataSet = false;
				addReturnParam(readerNavigator);
			}
		}
		...
		{
			aCommand.CommandText = getTextWithoutCDataBoundries(readerNavigator.getEntityContent());
		}

		/// <summary>
		/// removes boundries of a cdata section from a string which
		/// is a cdata section. 
		/// </summary>
		/// <param name="strCDataSection">CData section bound string (with &lt;![CData[ and ]]&gt;)</param>
		/// <returns>the string without the bounds</returns>
		private string getTextWithoutCDataBoundries(string strCDataSection) 
		{
			string strResult;
			int nCDataIndex = strCDataSection.IndexOf(CDATA_BOUND_START);
			int nCDataStart;
			int nCDataEnd;
			
			if(-1 == nCDataIndex) 
				strResult = strCDataSection;
			else 
			{
				nCDataStart = nCDataIndex + CDATA_BOUND_START.Length;
				nCDataEnd = strCDataSection.LastIndexOf(CDATA_BOUND_END);
				strResult = strCDataSection.Substring(nCDataStart,nCDataEnd - nCDataStart);
			}
