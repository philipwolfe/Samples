...
namespace Windows.Forms.Reports.Viewer
{
	#region MainDlg
	public class MainDlg : System.Windows.Forms.Form
	{
		...
		private Windows.Forms.Reports.ReportLibrary.UserRep userRep;
		...
		private void InitializeComponent()
		{
			...
			this.userRep.Location = new System.Drawing.Point(163, 32);
			...
			this.userRep.Size = new System.Drawing.Size(229, 234);
			this.userRep.TabIndex = 7;
			this.userRep.Text = "userRep";
			...
			// imageExport1
			// 
			this.imageExport1.FileName = null;
			...
			// htmlExport1
			// 
			this.htmlExport1.FileName = null;
			...
			// excelExport1
			// 
			this.excelExport1.FileName = null;
		...
		private void PreviewForm_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					reg=Registry.ClassesRoot.OpenSubKey("."+"SDR",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("."+"SDR");
					reg.SetValue("","ReportViewer");

					reg=Registry.ClassesRoot.OpenSubKey("ReportViewer",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("ReportViewer");
					reg.SetValue("","Report Viewer");

					reg=Registry.ClassesRoot.OpenSubKey("ReportViewer\\DefaultIcon",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("ReportViewer\\DefaultIcon");
					reg.SetValue("","\""+Application.ExecutablePath+"\""+",0");

					reg=Registry.ClassesRoot.OpenSubKey("ReportViewer\\Shell\\Open\\Command",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("ReportViewer\\Shell\\Open\\Command");
					reg.SetValue("","\""+Application.ExecutablePath+"\""+" %1");

					reg=Registry.ClassesRoot.OpenSubKey("ReportViewer\\Shell\\Print\\Command",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("ReportViewer\\Shell\\Print\\Command");
					reg.SetValue("","\""+Application.ExecutablePath+"\""+" /p %1");

					reg=Registry.ClassesRoot.OpenSubKey("ReportViewer\\Shell\\View\\Command",true);
					if(reg==null)
						reg=Registry.ClassesRoot.CreateSubKey("ReportViewer\\Shell\\View\\Command");
			...
			if(MainArray.Length>0)
			{
				...
				{
					if(ReportLibrary.Generic.CT(MainArray[i],"/p"))
					{
						ftoprint=true;
					}
					else if(ReportLibrary.Generic.CT(MainArray[i],"/v"))
					{
						ftoview=true;
					}
					else
				...
				userRep.SourceRep.LoadReport(filename,userRep.FileSystem,userRep);
				...
				if(ftoprint)
				{
					menuItem5_Click(this,EventArgs.Empty);
					Close();
				}
				if(ftoview)
				{
		...
		{
			if(userRep.SourceRep.BandCount>0)
			{
		...
		private void menuItem19_Click(object sender, System.EventArgs e)
		{
			...
			ReportLibrary.Generic.ZoomFactor=1;
			userRep.SourceRep.Save(userRep.FileSystem.FileName,userRep.FileSystem,userRep.Title);
