...
{
	/// <summary>
	/// Summary description for MyPointF.
	/// </summary>
	public struct MyPointF
	{
		...
		public static MyPointF operator+(MyPointF p1, Vector vec)
		{
		...
		public MyPointF Rotate(float angleInDegrees)
		{
			...
			double angleInRadians = angleInDegrees / (180F / Math.PI);
			return(new MyPointF(
				(float) (point.X * Math.Cos(angleInRadians) - point.Y * Math.Sin(angleInRadians)), 
				(float) (point.X * Math.Sin(angleInRadians) + point.Y * Math.Cos(angleInRadians))));
		}
