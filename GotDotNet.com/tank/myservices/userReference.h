...
template <typename TParentClass>  class CUserReference :
	public CBaseElement<CUserReference>
{
public:	
	CLocalizableString<CUserReference>	name;
	CString								puid;
	CString								email;
    CCat<CUserReference>				cat;
	...
	void
	SetFrom(CUserReference<TParentClass> *from)
	{
	...
	void 
	OnDoneChain(CCat<CUserReference> *const c)
	{
		cat.SetFrom(c);
	}

	void 
	OnDoneChain(CLocalizableString<CUserReference> *const c)
	{
		name.SetFrom(c);
	}
	...
	BEGIN_XMLTAG_MAP(CUserReference)
		XMLTAG_CLASS("cat",			CCat<CUserReference>)
		XMLTAG_ENTRY("puid",		set_puid)
		XMLTAG_ENTRY("email",		set_email)
		XMLTAG_CLASS("name",		CLocalizableString<CUserReference>)
	END_XMLTAG_MAP()
	...
	BEGIN_REPLACEMENT_METHOD_MAP(CUserReference)		
