        ...
        static void DemonstrateProblemUsingCanThrowAndMask()
        {
            ...
            try
            {
                ...
                using (CalculatorClient client = new CalculatorClient())
                {
                    ...
                    throw new ApplicationException("Hope this Exception wasn't important, because "+
                                                      "it might be masked by the Close Exception.");

                    // The following line calls Dispose on the client.  Dispose and Close are the
                    // same thing, and the Close is not successful because the server Aborted the
                    // channel.  This masks the ApplicationException above so nobody outside the
                    // "using" block will see it.
                }
            }
            catch (ApplicationException)
            {
                Console.WriteLine("We do not come here because the ApplicationException is masked.");
            }
            catch (CommunicationException e)
            {
                Console.WriteLine("Got {0}", e.GetType());
