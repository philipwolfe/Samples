...
namespace Microsoft.ServiceModel.Samples.Channels
{
    ...
    class UdpChannelListener : ChannelListenerBase<IInputChannel>
    {
            ...
            : base(context.Binding)
        {
            ...
            {
                this.messageEncoderFactory = messageEncoderBindingElement[0].CreateMessageEncoderFactory();
            }
            else
            {
            ...
            this.InitializeUri(context.ListenUriBaseAddress, context.ListenUriRelativeAddress, context.ListenUriMode);
            #endregion
        }
        ...
        #region IListenerFactory_Methods
        ...
        protected override void OnOpen(TimeSpan timeout)
        {
            ...
            // TODO: Should check whether the settings of the found transport manager matches
            ...
            lock (this.ThisLock)
            {
                ...
                if (IPAddress.TryParse(host, out ipAddress))
                {
                    Socket socket = UdpSocketListener.CreateListenSocket(
                        new IPEndPoint(ipAddress, 0), this.multicast);
                ...
                {
                    Socket socket = UdpSocketListener.CreateListenSocket(
                        new IPEndPoint(IPAddress.Any, 0), this.multicast);
                    port = ((IPEndPoint)socket.LocalEndPoint).Port;
                    listenSockets.Add(socket);
                    ...
                    if (Socket.OSSupportsIPv6)
                    {
                        listenSockets.Add(UdpSocketListener.CreateListenSocket(
                            new IPEndPoint(IPAddress.IPv6Any, port), this.multicast));
                    }
                }
            }
        ...
        protected override IInputChannel OnAcceptChannel(TimeSpan timeout)
        {
            ...
            if (this.channelQueue.TryDequeue(timeout, out channel))
            {
                return channel;
            }
            else
        ...
        protected override IAsyncResult OnBeginAcceptChannel(TimeSpan timeout, AsyncCallback callback, object state)
        {
            ...
            return this.channelQueue.BeginTryDequeue(timeout, callback, state);
        }

        protected override IInputChannel OnEndAcceptChannel(IAsyncResult result)
        {
            IInputChannel channel;
            if (this.channelQueue.EndTryDequeue(result, out channel))
            {
                return channel;
            }
            else
