...
namespace Microsoft.ServiceModel.Samples.Hosting
{
    class HostedUdpTransportConfigurationImpl
    {
        ...
        HostedUdpTransportConfigurationImpl()
        {
            ...
            {
                if (string.Compare(binding.Protocol, UdpConstants.UdpProtocolId, StringComparison.OrdinalIgnoreCase) == 0)
                {
            ...
            {
                throw new InvalidOperationException(string.Format("No binding is found for the protocol '{0}'", UdpConstants.UdpProtocolId));
            }
            ...
            foreach (string bindingInfo in bindingInfoList)
            {
                ...
                if (!int.TryParse(bindingInfo, out port))
                {
                    throw new InvalidDataException(string.Format("The binding information '{0}' is invalid for the protocol '{1}'",
                        bindingInfo, UdpConstants.UdpProtocolId));
                }

                // TODO: Use the right host for the Uri.
                UriBuilder builder = new UriBuilder(UdpConstants.UriSchemeNetUdp, "localhost", port,
                    HostingEnvironment.ApplicationVirtualPath);

                baseAddresses.Add(builder.Uri);
            }

            // TODO: add support with multiple transport managers.
