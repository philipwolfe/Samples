...
IF NOT ERRORLEVEL 1 (
    IF EXIST %windir%\system32\inetsrv\appcmd.exe (
        %windir%\system32\inetsrv\appcmd.exe add app /site.name:"Default Web Site" /path:/ServiceModelSamples /physicalPath:%systemdrive%\inetpub\wwwroot\ServiceModelSamples /applicationPool:"ASP.NET v4.0"
    ) ELSE (
    ...
    IF EXIST %SystemDrive%\inetpub\adminscripts\adsutil.vbs (
	    ...
	    cscript.exe %SystemDrive%\inetpub\adminscripts\adsutil.vbs set w3svc/1/root/ServiceModelSamples/AppPoolId "ASP.NET v4.0"
    ) ELSE (
