...
namespace Microsoft.Samples.Compensation.CustomCompensation
{
    ...
    class Program
    {
        ...
        static WorkflowElement TruckRentalAgency()
        {
            ...
            Variable<int> truckRentalRate = new Variable<int> { Default = ValueExpression.CreateLiteral(500) };
            
            return new Parallel
            {
                // ends when either the customer cancels the order or the reservation date is now
                CompletionCondition = ValueExpression.CreateLiteral(true),
                ...
                Branches =  
                {
                    ...
                    // Using CompensationScope to delimit a compensation zone that confirms if Body completes with success
                    new TryCatch
                    {
                                    ...
                                    new CompensableActivity
                                    {
                                        ...
                                        CompensationHandler = new Sequence // used to define the custom compensation logic
                                        {
                                            Activities = 
                                            {
                                                ...
                                                new If
                                                {
                                                    Condition = ValueExpression.CreateLiteral(true), // simulate a non-premium customer 
                                                                                                     // with less than 10 days to reservation date
                                                    Then = new Confirm
                                                    {
