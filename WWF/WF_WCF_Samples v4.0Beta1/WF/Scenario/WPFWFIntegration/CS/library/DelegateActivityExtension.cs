...
namespace Microsoft.Activities.Samples
{
    ...
    public class DelegateActivityExtension : MarkupExtension
    {
        ...
        public WorkflowElement Activity { get; set; }
        ...
        object GenericInvoke(params object[] args)
        {
            ...
            {
                symbolResolver.Add(Argument.ReturnValueName, new ValueHolder<object>());
            }

            //using WorkflowInstance in this sample for Beta1.
            //The two caveats of our usage of WorkflowInstance here are:
            //1) The workflow will run under the WPF SynchronizationContext so this delegate will complete before the workflow completes
            //2) Any exceptions from the WorkflowInstance should be marshalled back using the WPF dispatcher.            
            //In RTM, this will use the WorkflowInvoker 
            WorkflowInstance instance = new WorkflowInstance(this.Activity, new Dictionary<string, object>(), SynchronizationContext.Current, symbolResolver);

            instance.Extensions.Add(symbolResolver);

            instance.OnCompleted = OnWorkflowCompleted;
            instance.OnAborted = OnWorkflowAborted;
        ...
        void OnWorkflowCompleted(WorkflowCompletedEventArgs e)
        {
        ...
        void OnWorkflowAborted(WorkflowAbortedEventArgs e)
        {
            Application.Current.Dispatcher.BeginInvoke(
